  ...
  //   logger.info(`file: ${fname},post allQA is called`);
    ...
    async function getAllQandA()
    {
      ...
      await ConnectToDb().then(async (dbConnection)=>{
        if(dbConnection){
          await ExecuteQuery(dbConnection,`select assessmentid, date from assessment where interviewid = ${interviewid}`)
          
          .then(async(value)=>{
              console.log(value)
              assessmentid=value[0].assessmentid;
              console.log(assessmentid);
              await ExecuteQuery(dbConnection, `select question,questions.queId,answer,answerkeywords,answer.ansId, questions.cmpid, complexity.name, skill.skillName , questions.skillid, score, note from assessmentdetails
